...
export default function ParentScreens({ v }) {
  return (
    <>
        ...
        <div style={css(`padding-bottom:120px;animation:ybup .3s ease`)}>
          <div style={css(`padding:6px 20px 20px;background:linear-gradient(180deg,#eaf6f8 0%,#fcfcfc 100%)`)}>
            <div style={css(`display:flex;align-items:center;justify-content:space-between`)}>
              <div><div style={css(`font-size:12px;color:#617789`)}>Phụ huynh</div><div style={css(`font-size:17px;font-weight:700;color:#195658`)}>{v.parentName}</div></div>
            ...
            <div style={css(`margin-top:14px;display:flex;gap:10px`)}>
              {v.children.map((c, i) => (
                <div key={i} onClick={c.onClick} style={css(`flex:1;display:flex;align-items:center;gap:10px;padding:11px;border:${c.border};background:${c.bg};border-radius:16px;cursor:pointer;transition:all .15s`)}>
                  ...
                  <span style={css(`font-size:12.5px;font-weight:${c.weight};color:#195658;line-height:1.3`)}>{c.name}</span>
                </div>
              ))}
            </div>
            <div style={css(`margin-top:14px;display:grid;grid-template-columns:1fr 1fr;gap:11px`)}>
              {v.childStats.map((st, i) => (
                <div key={i} style={css(`padding:14px;border-radius:18px;background:${st.bg}`)}><div style={css(`font-size:26px;font-weight:700;color:${st.color};line-height:1`)}>{st.value}</div><div style={css(`font-size:11.5px;color:#617789;margin-top:5px`)}>{st.label}</div></div>
              ))}
            </div>
          </div>
          <div style={css(`padding:4px 20px 0`)}>
            <div style={css(`padding:16px;border:1px solid #ddeaf0;border-radius:20px;background:#fff`)}>
              <div style={css(`display:flex;align-items:center;justify-content:space-between`)}><span style={css(`font-size:13.5px;font-weight:600;color:#195658`)}>Tiến độ học tập</span><span style={css(`font-size:13px;font-weight:700;color:#00708f`)}>{v.childPath}</span></div>
              ...
              <div style={css(`margin-top:9px;font-size:11.5px;color:#617789`)}>{v.childMissions} mission hoàn thành trong tuần này</div>
            </div>
            <div style={css(`margin-top:14px;padding:16px;border-radius:20px;background:#195658;color:#fff;position:relative;overflow:hidden`)}>
              ...
              <div style={css(`position:relative`)}>
                <div style={css(`font-size:11.5px;letter-spacing:.06em;text-transform:uppercase;opacity:.7`)}>Gợi ý cho tuần tới</div>
                <div style={css(`font-size:14px;line-height:1.6;margin-top:7px;text-wrap:pretty`)}>{v.nextAction}</div>
                <div onClick={v.toPReport} style={css(`margin-top:14px;height:38px;padding:0 16px;border-radius:999px;background:#fff;color:#195658;font-size:13px;font-weight:600;display:inline-flex;align-items:center;cursor:pointer`)}>Tạo báo cáo tuần</div>
              </div>
            </div>
            <div style={css(`margin-top:16px;display:flex;flex-direction:column;gap:10px`)}>
              <div onClick={v.toPChild} style={css(`display:flex;align-items:center;gap:12px;padding:14px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;cursor:pointer`)}><div style={css(`width:34px;height:34px;flex:none;border-radius:11px;background:#eaf6f8;display:flex;align-items:center;justify-content:center`)}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#00708f" strokeWidth="1.8" strokeLinecap="round"><path d="M4 20V10.5M10 20V4.5M16 20v-7M21.5 20h-19"/></svg></div><div style={css(`flex:1`)}><div style={css(`font-size:13.5px;font-weight:600;color:#195658`)}>Theo dõi học tập</div><div style={css(`font-size:11.5px;color:#617789;margin-top:2px`)}>Kết quả theo lớp và bài tập gần đây</div></div><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#8ba0ae" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9.5 5l7 7-7 7"/></svg></div>
              <div onClick={v.toPSafety} style={css(`display:flex;align-items:center;gap:12px;padding:14px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;cursor:pointer`)}><div style={css(`width:34px;height:34px;flex:none;border-radius:11px;background:#eaf6f8;display:flex;align-items:center;justify-content:center`)}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#00708f" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3.5l7 3v5.2c0 4.3-2.9 7.6-7 8.8-4.1-1.2-7-4.5-7-8.8V6.5l7-3z"/></svg></div><div style={css(`flex:1`)}><div style={css(`font-size:13.5px;font-weight:600;color:#195658`)}>Cài đặt an toàn</div><div style={css(`font-size:11.5px;color:#617789;margin-top:2px`)}>Social, leaderboard, AI tutor và giới hạn thời gian</div></div><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#8ba0ae" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9.5 5l7 7-7 7"/></svg></div>
            </div>
          </div>
        </div>
      )}
      ...
      {v.isPChild && (
        <div style={css(`padding:6px 20px 120px;animation:ybup .3s ease`)}>
          <div style={css(`display:flex;align-items:center;gap:13px`)}>
            ...
            <div><div style={css(`font-size:18px;font-weight:700;color:#195658`)}>{v.childName}</div><div style={css(`font-size:12px;color:#617789;margin-top:2px`)}>{v.childMeta}</div></div>
          </div>
          <div style={css(`margin-top:20px;font-size:14.5px;font-weight:600;color:#195658`)}>Kết quả theo lớp</div>
          <div style={css(`margin-top:12px;display:flex;flex-direction:column;gap:11px`)}>
            {v.childClasses.map((c, i) => (
              <div key={i} style={css(`padding:14px;border:1px solid #ddeaf0;border-radius:18px;background:#fff`)}>
                <div style={css(`display:flex;align-items:center;justify-content:space-between`)}><div style={css(`font-size:13.5px;font-weight:700;color:#195658;letter-spacing:.02em`)}>{c.name}</div><div style={css(`font-size:19px;font-weight:700;color:#00708f`)}>{c.score}</div></div>
                <div style={css(`font-size:11.5px;color:#617789;margin-top:4px`)}>{c.teacher} · {c.submitted}</div>
              </div>
            ))}
          </div>
          <div style={css(`margin-top:20px;font-size:14.5px;font-weight:600;color:#195658`)}>Bài tập gần đây</div>
          <div style={css(`margin-top:12px;display:flex;flex-direction:column;gap:11px`)}>
            {v.childAssignments.map((a, i) => (
              <div key={i} style={css(`padding:14px;border:1px solid #ddeaf0;border-radius:18px;background:#fff`)}>
                ...
                <div style={css(`margin-top:8px;display:flex;align-items:center;gap:9px`)}>
                  ...
                  <span style={css(`font-size:11px;color:#8ba0ae`)}>{a.cls}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {v.isPReport && (
        <div style={css(`padding:6px 20px 120px;animation:ybup .3s ease`)}>
          <div style={css(`font-size:22px;font-weight:700;color:#195658`)}>Báo cáo tuần</div>
          <div style={css(`font-size:13px;color:#617789;margin-top:3px`)}>{v.reportPeriod}</div>
          <div style={css(`margin-top:16px;padding:16px;border:1px solid #ddeaf0;border-radius:20px;background:#fff`)}>
            <div style={css(`font-size:13px;line-height:1.7;color:#25475a;text-wrap:pretty`)}>{v.reportSummary}</div>
            <div style={css(`margin-top:14px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:9px`)}>
              {v.reportMetrics.map((m, i) => (
                <div key={i} style={css(`padding:12px 8px;border-radius:14px;background:#edf7f9;text-align:center`)}><div style={css(`font-size:19px;font-weight:700;color:#00708f;line-height:1`)}>{m.value}</div><div style={css(`font-size:10px;color:#617789;margin-top:4px;line-height:1.3`)}>{m.label}</div></div>
              ))}
            </div>
          </div>
          <div style={css(`margin-top:18px;display:flex;align-items:center;gap:8px`)}><div style={css(`width:8px;height:8px;border-radius:999px;background:#22c55e`)}></div><div style={css(`font-size:14px;font-weight:600;color:#195658`)}>Điểm mạnh</div></div>
          <div style={css(`margin-top:10px;display:flex;flex-direction:column;gap:9px`)}>
            {v.reportStrengths.map((r, i) => (
              <div key={i} style={css(`padding:13px;border:1px solid #cdecd8;border-radius:14px;background:#f6fdf8;font-size:12.5px;line-height:1.55;color:#25475a;text-wrap:pretty`)}>{r.text}</div>
            ))}
          </div>
          <div style={css(`margin-top:18px;display:flex;align-items:center;gap:8px`)}><div style={css(`width:8px;height:8px;border-radius:999px;background:#f59e0b`)}></div><div style={css(`font-size:14px;font-weight:600;color:#195658`)}>Cần cải thiện</div></div>
          <div style={css(`margin-top:10px;display:flex;flex-direction:column;gap:9px`)}>
            {v.reportImprovements.map((r, i) => (
              <div key={i} style={css(`padding:13px;border:1px solid #f6e2bd;border-radius:14px;background:#fffaf1;font-size:12.5px;line-height:1.55;color:#25475a;text-wrap:pretty`)}>{r.text}</div>
            ))}
          </div>
          <div onClick={v.toPMessages} style={css(`margin-top:20px;height:52px;border-radius:16px;background:#00aaab;color:#fff;font-size:15px;font-weight:600;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 8px 20px rgba(0,170,171,.3)`)}>Trao đổi với giáo viên</div>
        </div>
      )}
      ...
      {v.isPMessages && (
        <div style={css(`padding:6px 20px 120px;animation:ybup .3s ease`)}>
          <div style={css(`display:flex;align-items:flex-start;justify-content:space-between`)}>
            <div><div style={css(`font-size:22px;font-weight:700;color:#195658`)}>Liên hệ giáo viên</div><div style={css(`font-size:13px;color:#617789;margin-top:3px;text-wrap:pretty`)}>Gửi tin nhắn cho giáo viên đang dạy con và theo dõi phản hồi tại một nơi.</div></div>
          </div>
            ...
            {v.convos.map((c, i) => (
              <div key={i} onClick={c.onClick} style={css(`padding:14px;border:1px solid #ddeaf0;border-radius:18px;background:#fff;cursor:pointer`)}>
                <div style={css(`display:flex;align-items:center;gap:11px`)}>
                  ...
                  <div style={css(`flex:1;min-width:0`)}><div style={css(`font-size:13.5px;font-weight:600;color:#195658`)}>{c.teacher}</div><div style={css(`font-size:11px;color:#617789;margin-top:2px`)}>{c.cls}</div></div>
                  <div style={css(`display:flex;flex-direction:column;align-items:flex-end;gap:5px`)}>
                    <span style={css(`font-size:10.5px;color:#8ba0ae`)}>{c.when}</span>
      ...
      {v.isPThread && (
        <div style={css(`min-height:100%;display:flex;flex-direction:column;animation:ybup .3s ease`)}>
          <div style={css(`padding:6px 20px 14px;border-bottom:1px solid #ddeaf0;display:flex;align-items:center;gap:12px`)}>
            ...
            <div><div style={css(`font-size:14.5px;font-weight:600;color:#195658`)}>Cô Phạm Thu Trang</div><div style={css(`font-size:11px;color:#617789`)}>SINH HỌC 8A1 · Trao đổi học tập</div></div>
          </div>
          <div style={css(`flex:1;padding:16px 20px;display:flex;flex-direction:column;gap:12px`)}>
            {v.thread.map((m, i) => (
              <div key={i} style={css(`display:flex;flex-direction:column;align-items:${m.align}`)}>
                ...
                <span style={css(`font-size:10.5px;color:#8ba0ae;margin-top:5px`)}>{m.at}</span>
              </div>
            ))}
            {v.sent && (
              <div style={css(`display:flex;flex-direction:column;align-items:flex-end;animation:ybup .3s ease`)}>
                <div style={css(`max-width:82%;padding:12px 14px;border:1px solid #00aaab;background:#00aaab;color:#fff;border-radius:18px;font-size:13px;line-height:1.6`)}>Cảm ơn cô, tối nay em sẽ cho cháu xem lại mô hình 3D và nhắc nộp bài đúng hạn.</div>
                <span style={css(`font-size:10.5px;color:#8ba0ae;margin-top:5px`)}>Vừa gửi · đã gửi</span>
              </div>
            )}
          </div>
          <div style={css(`padding:12px 20px 26px;border-top:1px solid #ddeaf0;display:flex;align-items:center;gap:10px;background:#fcfcfc`)}>
            <div style={css(`flex:1;min-height:48px;padding:13px 15px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;font-size:13px;color:#8ba0ae`)}>Nhập nội dung tin nhắn...</div>
      ...
      {v.isPSafety && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          <div style={css(`display:flex;align-items:center;gap:12px`)}>
            ...
            <div style={css(`font-size:18px;font-weight:700;color:#195658`)}>Cài đặt an toàn</div>
          </div>
          <div style={css(`margin-top:8px;font-size:12.5px;line-height:1.55;color:#617789;text-wrap:pretty`)}>Quản lý social, leaderboard, AI tutor và giới hạn thời gian học.</div>
          <div style={css(`margin-top:18px;display:flex;flex-direction:column;gap:11px`)}>
            {v.safetyRows.map((r, i) => (
              <div key={i} onClick={r.onClick} style={css(`display:flex;align-items:center;gap:12px;padding:15px;border:1px solid #ddeaf0;border-radius:18px;background:#fff;cursor:pointer`)}>
                <div style={css(`flex:1;min-width:0`)}><div style={css(`font-size:13.5px;font-weight:600;color:#195658`)}>{r.label}</div><div style={css(`font-size:11.5px;color:#617789;margin-top:3px;line-height:1.45`)}>{r.desc}</div></div>
          ...
          <div style={css(`margin-top:20px;font-size:14px;font-weight:600;color:#195658`)}>Giới hạn màn hình mỗi ngày</div>
          ...
          <div style={css(`margin-top:12px;padding:13px 14px;border-radius:14px;background:#edf7f9;font-size:12px;line-height:1.55;color:#617789;text-wrap:pretty`)}>Khi đạt giới hạn, con sẽ thấy thông báo “Hết thời gian sử dụng” và không thể tiếp tục học trong ngày.</div>
          <div onClick={v.toPOverview} style={css(`margin-top:22px;height:54px;border-radius:16px;background:#00aaab;color:#fff;font-size:15.5px;font-weight:600;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 10px 22px rgba(0,170,171,.28)`)}>Lưu cài đặt</div>
        </div>
      )}
    </>
  );
