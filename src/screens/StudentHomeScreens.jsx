    ...
    <>
      {v.isHome && (
        <div style={css(`padding-bottom:120px;animation:ybup .3s ease`)}>
          <div style={css(`padding:6px 20px 20px;background:linear-gradient(180deg,#eaf6f8 0%,#fcfcfc 100%)`)}>
            <div style={css(`display:flex;align-items:center;justify-content:space-between`)}>
              <div style={css(`display:flex;align-items:center;gap:11px`)}>
                ...
                <div><div style={css(`font-size:12px;color:#455771`)}>{v.t(`Chào buổi sáng`)}</div><div style={css(`font-size:15.5px;font-weight:600;color:#455771`)}>{v.userName}</div></div>
              </div>
              <div style={css(`display:flex;align-items:center;gap:9px`)}>
                <div style={css(`display:flex;align-items:center;gap:5px;height:34px;padding:0 11px;border-radius:999px;background:#fff;border:1px solid #ddeaf0`)}><img src="/assets/learning-points.svg" alt="" style={css(`width:16px;height:16px`)}/><span style={css(`font-size:13px;font-weight:600;color:#455771`)}>1.240</span></div>
              ...
              {v.featured.map((l, i) => (
                <div key={i} onClick={l.onClick} style={css(`flex:none;width:236px;border:1px solid #ddeaf0;border-radius:22px;background:#fff;overflow:hidden;cursor:pointer;box-shadow:0 4px 14px rgba(25,86,88,.05)`)}>
                  <div style={css(`height:126px;position:relative;background:${l.tintBg}`)}>
                    ...
                    <div style={css(`position:absolute;left:10px;top:10px;height:24px;padding:0 10px;border-radius:999px;background:rgba(255,255,255,.92);font-size:10.5px;font-weight:600;color:#455771;display:flex;align-items:center`)}>{l.typeLabel}</div>
                  </div>
                  <div style={css(`padding:12px 14px 14px`)}>
                    <div style={css(`font-size:14.5px;font-weight:600;color:#455771;line-height:1.35;text-wrap:pretty`)}>{l.title}</div>
                    <div style={css(`font-size:11.5px;color:#455771;margin-top:5px`)}>{l.meta}</div>
                    <div style={css(`margin-top:10px;display:flex;align-items:center;justify-content:space-between`)}>
                      <div style={css(`display:flex;align-items:center;gap:4px`)}><img src="/assets/lesson/star.svg" alt="" style={css(`width:13px;height:13px`)}/><span style={css(`font-size:12px;font-weight:600;color:#455771`)}>{l.rating}</span><span style={css(`font-size:11.5px;color:#455771`)}>({l.reviews})</span></div>
          ...
          <div style={css(`padding:22px 20px 0`)}>
            <div style={css(`font-size:16px;font-weight:600;color:#455771`)}>{v.t(`Tuần này của bạn`)}</div>
            <div style={css(`margin-top:12px;display:grid;grid-template-columns:1fr 1fr;gap:12px`)}>
              {v.stats.map((s, i) => (
                <div key={i} onClick={s.onClick} className="yb-press" style={css(`border:1px solid #ddeaf0;border-radius:20px;background:#fff;padding:14px;cursor:pointer;transition:all .15s`)}>
                  ...
                  <div style={css(`font-size:24px;font-weight:700;color:#455771;margin-top:10px;line-height:1`)}>{s.value}</div>
                  <div style={css(`font-size:12px;color:#455771;margin-top:4px`)}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            {v.lessons.map((l, i) => (
              <div key={i} style={css(`border:1px solid #ddeaf0;border-radius:20px;background:#fff;overflow:hidden;display:flex;flex-direction:${v.cardDir};box-shadow:0 3px 12px rgba(25,86,88,.045)`)}>
                ...
                <div style={css(`padding:11px 12px 12px;flex:1;min-width:0`)}>
                  <div onClick={l.onView} style={css(`font-size:13.5px;font-weight:600;color:#455771;line-height:1.35;text-wrap:pretty;cursor:pointer`)}>{l.title}</div>
                  <div style={css(`font-size:11px;color:#455771;margin-top:4px`)}>{l.meta}</div>
        ...
        <>
          <div style={css(`padding-bottom:110px;animation:ybup .3s ease`)}>
            ...
            <div style={css(`padding:18px 20px 0`)}>
              ...
              <div style={css(`font-size:21px;font-weight:700;color:#455771;line-height:1.3;margin-top:12px;text-wrap:pretty`)}>{v.t(`Cấu tạo tế bào thực vật`)}</div>
              <div style={css(`margin-top:10px;display:flex;align-items:center;gap:14px;font-size:12.5px;color:#455771`)}>
                <div style={css(`display:flex;align-items:center;gap:4px`)}><img src="/assets/lesson/star.svg" alt="" style={css(`width:14px;height:14px`)}/><span style={css(`font-weight:600;color:#455771`)}>4.8</span>(126)</div>
              ...
              <div style={css(`margin-top:16px;display:flex;align-items:center;gap:11px;padding:13px;border:1px solid #ddeaf0;border-radius:18px;background:#fff`)}>
                ...
                <div style={css(`flex:1`)}><div style={css(`font-size:14px;font-weight:600;color:#455771`)}>{v.t(`Cô Phạm Thu Trang`)}</div><div style={css(`font-size:11.5px;color:#455771`)}>{v.t(`THCS Chu Văn An · 24 học liệu`)}</div></div>
              ...
              {v.detailTabOverview && (
                <div style={css(`margin-top:16px`)}>
                  <div style={css(`font-size:13.5px;line-height:1.65;color:#455771;text-wrap:pretty`)}>{v.t(`Bài giảng mô phỏng 3D toàn bộ cấu trúc tế bào thực vật: thành tế bào, màng sinh chất, lục lạp, không bào trung tâm và nhân. Học sinh có thể xoay, tách lớp và phóng to từng bào quan để quan sát chi tiết.`)}</div>
                  <div style={css(`margin-top:16px;display:flex;flex-direction:column;gap:9px`)}>
                    {v.detailFacts.map((f, i) => (
                      <div key={i} style={css(`display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border:1px solid #ddeaf0;border-radius:14px;background:#fff`)}><span style={css(`font-size:12.5px;color:#455771`)}>{f.k}</span><span style={css(`font-size:13px;font-weight:600;color:#455771`)}>{f.v}</span></div>
                    ))}
                  </div>
                </div>
              )}
                ...
                <div style={css(`margin-top:16px;display:flex;flex-direction:column;gap:9px`)}>
                  {v.steps.map((s, i) => (
                    <div key={i} onClick={s.onClick} style={css(`display:flex;align-items:center;gap:12px;padding:13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;cursor:pointer`)}>
                      ...
                      <div style={css(`flex:1;min-width:0`)}><div style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{s.title}</div><div style={css(`font-size:11.5px;color:#455771;margin-top:2px`)}>{s.kind} · {s.dur}</div></div>
                  ...
                  {v.reviews.map((r, i) => (
                    <div key={i} style={css(`padding:14px;border:1px solid #ddeaf0;border-radius:16px;background:#fff`)}>
                      <div style={css(`display:flex;align-items:center;gap:10px`)}>
                        ...
                        <div style={css(`flex:1`)}><div style={css(`font-size:13px;font-weight:600;color:#455771`)}>{r.name}</div><div style={css(`font-size:11px;color:#455771`)}>{r.when}</div></div>
                      ...
                      <div style={css(`font-size:12.5px;line-height:1.6;color:#455771;margin-top:9px;text-wrap:pretty`)}>{r.text}</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div style={css(`position:absolute;left:0;right:0;bottom:0;padding:12px 20px 22px;background:rgba(252,252,252,.94);backdrop-filter:blur(12px);border-top:1px solid #ddeaf0;display:flex;align-items:center;gap:12px;z-index:25`)}>
            <div><div style={css(`font-size:11px;color:#455771`)}>{v.t(`Giá học liệu`)}</div><div style={css(`display:flex;align-items:center;gap:5px`)}><img src="/assets/lesson/token.svg" alt="" style={css(`width:17px;height:17px`)}/><span style={css(`font-size:19px;font-weight:700;color:#00708f`)}>120</span></div></div>
      ...
      {v.isLearningActivity && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          <div style={css(`display:flex;align-items:center;gap:12px`)}>
            ...
            <div style={css(`font-size:18px;font-weight:700;color:#455771`)}>{v.t(`Bài đã học`)}</div>
          </div>
          <div style={css(`margin-top:18px;padding:16px;border-radius:20px;background:#eaf6f8`)}>
            ...
            <div style={css(`font-size:28px;font-weight:700;color:#455771;margin-top:3px`)}>{v.weekActivity.length} <span style={css(`font-size:14px;font-weight:500;color:#455771`)}>{v.t(`bài học`)}</span></div>
          </div>
            ...
            {v.weekActivity.map((a, i) => (
              <div key={i} style={css(`display:flex;align-items:center;gap:12px;padding:14px;border:1px solid #ddeaf0;border-radius:18px;background:#fff`)}>
                ...
                <div style={css(`flex:1;min-width:0`)}>
                  <div style={css(`font-size:13.5px;font-weight:600;color:#455771;line-height:1.35;text-wrap:pretty`)}>{v.t(a.title)}</div>
                  <div style={css(`font-size:11.5px;color:#455771;margin-top:3px`)}>{v.t(a.subject)} · {a.when} · {v.t(a.duration)}</div>
                </div>
      ...
      {v.isStudyTime && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          <div style={css(`display:flex;align-items:center;gap:12px`)}>
            <div onClick={v.toHome} style={css(`width:38px;height:38px;border-radius:999px;border:1px solid #ddeaf0;background:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer`)}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#195658" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 5L7.5 12l7 7"/></svg></div>
            <div style={css(`font-size:18px;font-weight:700;color:#455771`)}>{v.t(`Thời gian học`)}</div>
          </div>
          ...
          <div style={css(`margin-top:22px;font-size:14.5px;font-weight:600;color:#455771`)}>{v.t(`Phân bổ theo ngày`)}</div>
          <div style={css(`margin-top:14px;display:flex;align-items:flex-end;gap:8px;height:110px;padding:0 4px`)}>
            {v.dailyStudy.map((d, i) => (
              <div key={i} style={css(`flex:1;display:flex;flex-direction:column;align-items:center;gap:6px;height:100%;justify-content:flex-end`)}>
                ...
                <span style={css(`font-size:10.5px;color:#455771`)}>{d.day}</span>
              </div>
            ))}
          </div>

          <div style={css(`margin-top:22px;font-size:14.5px;font-weight:600;color:#455771`)}>{v.t(`Theo môn học`)}</div>
          <div style={css(`margin-top:12px;display:flex;flex-direction:column;gap:10px`)}>
            {v.subjectStudy.map((s, i) => (
              <div key={i} style={css(`display:flex;align-items:center;gap:12px;padding:13px 14px;border:1px solid #ddeaf0;border-radius:16px;background:#fff`)}>
                ...
                <div style={css(`flex:1;font-size:13.5px;font-weight:500;color:#455771`)}>{v.t(s.subject)}</div>
                <span style={css(`font-size:13px;font-weight:600;color:#455771`)}>{s.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {v.isLearningStreak && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          <div style={css(`display:flex;align-items:center;gap:12px`)}>
            <div onClick={v.toHome} style={css(`width:38px;height:38px;border-radius:999px;border:1px solid #ddeaf0;background:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer`)}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#195658" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 5L7.5 12l7 7"/></svg></div>
            <div style={css(`font-size:18px;font-weight:700;color:#455771`)}>{v.t(`Ngày liên tiếp`)}</div>
          </div>
          ...
          <div style={css(`margin-top:22px;font-size:14.5px;font-weight:600;color:#455771`)}>{v.t(`7 ngày gần nhất`)}</div>
          ...
          <div style={css(`margin-top:18px;display:flex;align-items:center;justify-content:space-between;padding:14px;border:1px solid #ddeaf0;border-radius:16px;background:#fff`)}>
            <span style={css(`font-size:13px;color:#455771`)}>{v.t(`Kỷ lục dài nhất`)}</span>
            <span style={css(`font-size:14px;font-weight:700;color:#455771`)}>{v.bestStreak} {v.t(`ngày`)}</span>
          </div>
      ...
      {v.isAverageScore && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          <div style={css(`display:flex;align-items:center;gap:12px`)}>
            <div onClick={v.toHome} style={css(`width:38px;height:38px;border-radius:999px;border:1px solid #ddeaf0;background:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer`)}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#195658" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 5L7.5 12l7 7"/></svg></div>
            <div style={css(`font-size:18px;font-weight:700;color:#455771`)}>{v.t(`Điểm trung bình`)}</div>
          </div>
          <div style={css(`margin-top:18px;padding:18px;border-radius:20px;background:#fdeef5;text-align:center`)}>
            ...
            <div style={css(`display:flex;align-items:center;justify-content:center;gap:6px;margin-top:4px`)}><img src="/assets/lesson/star.svg" alt="" style={css(`width:22px;height:22px`)}/><span style={css(`font-size:32px;font-weight:700;color:#455771`)}>8.4</span></div>
          </div>

          <div style={css(`margin-top:22px;font-size:14.5px;font-weight:600;color:#455771`)}>{v.t(`Kết quả gần đây`)}</div>
          <div style={css(`margin-top:12px;display:flex;flex-direction:column;gap:10px`)}>
            {v.recentScores.map((r, i) => (
              <div key={i} style={css(`padding:14px;border:1px solid #ddeaf0;border-radius:18px;background:#fff`)}>
                <div style={css(`display:flex;align-items:flex-start;justify-content:space-between;gap:10px`)}>
                  <div style={css(`flex:1;min-width:0`)}>
                    <div style={css(`font-size:13.5px;font-weight:600;color:#455771;line-height:1.35;text-wrap:pretty`)}>{v.t(r.title)}</div>
                    <div style={css(`font-size:11.5px;color:#455771;margin-top:3px`)}>{v.t(r.subject)} · {r.when}</div>
                  </div>
                  <div style={css(`font-size:18px;font-weight:700;color:#455771;flex:none`)}>{r.score}</div>
                </div>
