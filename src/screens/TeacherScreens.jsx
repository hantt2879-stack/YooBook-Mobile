    ...
    <>
          ...
          <div style={css(`padding:6px 20px 20px;background:linear-gradient(180deg,#eaf6f8 0%,#fcfcfc 100%)`)}>
            <div style={css(`display:flex;align-items:center;justify-content:space-between`)}>
              <div style={css(`display:flex;align-items:center;gap:11px`)}>
                ...
                <div><div style={css(`font-size:12px;color:#455771`)}>{v.t(`Giáo viên`)}</div><div style={css(`font-size:15.5px;font-weight:600;color:#455771`)}>{v.teacherName}</div></div>
              </div>
            ...
            <div style={css(`margin-top:16px;display:grid;grid-template-columns:1fr 1fr;gap:11px`)}>
              {v.tMetrics.map((m, i) => (
                <div key={i} style={css(`border:1px solid #ddeaf0;border-radius:20px;background:#fff;padding:14px`)}>
                  ...
                  <div style={css(`font-size:30px;font-weight:700;color:#455771;line-height:1;margin-top:8px`)}>{m.value}</div>
                  <div style={css(`font-size:10.5px;color:#455771;margin-top:5px;line-height:1.4`)}>{m.hint}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              {v.tClasses.map((c, i) => (
                <div key={i} onClick={c.onClick} style={css(`flex:none;width:212px;border-radius:20px;overflow:hidden;border:1px solid #ddeaf0;background:#fff;cursor:pointer`)}>
                  ...
                  <div style={css(`padding:12px 13px 13px`)}>
                    <div style={css(`font-size:11.5px;color:#455771`)}>{c.sub} · {c.students} {v.t(`học sinh`)}</div>
                    ...
                    <div style={css(`font-size:11px;color:#455771;margin-top:6px`)}>{v.t(`Tiến độ`)} {c.progress} · {v.t(`mã`)} {c.code}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={css(`margin-top:14px;display:flex;flex-direction:column;gap:10px`)}>
              {v.tSubmissions.map((x, i) => (
                <div key={i} onClick={x.onClick} style={css(`display:flex;align-items:center;gap:11px;padding:12px 13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;cursor:pointer`)}>
                  ...
                  <div style={css(`flex:1;min-width:0`)}><div style={css(`font-size:13px;font-weight:600;color:#455771`)}>{x.name}</div><div style={css(`font-size:11px;color:#455771;margin-top:2px`)}>{x.at} · {x.attempt}</div></div>
            ...
            <div style={css(`margin-top:14px;display:flex;flex-direction:column;gap:10px`)}>
              {v.tAssignments.map((a, i) => (
                <div key={i} onClick={a.onClick} style={css(`padding:13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;cursor:pointer`)}>
                  <div style={css(`font-size:13px;font-weight:600;color:#455771;line-height:1.4;text-wrap:pretty`)}>{a.title}</div>
                  <div style={css(`font-size:11px;color:#455771;margin-top:4px`)}>{a.cls} · {v.t(`hạn`)} {a.due}</div>
              ...
              {v.tUpcoming.map((a, i) => (
                <div key={i} onClick={a.onClick} style={css(`display:flex;align-items:center;gap:10px;padding:12px 13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;cursor:pointer`)}>
                  ...
                  <div style={css(`flex:1;min-width:0`)}>
                    <div style={css(`font-size:13px;font-weight:600;color:#455771;line-height:1.4;text-wrap:pretty`)}>{a.title}</div>
                    <div style={css(`font-size:11px;color:#455771;margin-top:3px`)}>{a.cls} · {v.t(`hạn`)} {a.due}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {v.isTCreateClass && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          ...
          <div style={css(`margin-top:20px;font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Môn học`)}</div>
          <div style={css(`margin-top:10px;display:grid;grid-template-columns:1fr 1fr;gap:9px`)}>
            {v.ccSubjects.map((o, i) => (
              <div key={i} onClick={o.onClick} style={css(`height:46px;border:${o.selected ? "1.6px solid #00aaab" : "1px solid #ddeaf0"};background:${o.selected ? "#eaf6f8" : "#fff"};color:${o.selected ? "#00708f" : "#455771"};border-radius:14px;font-size:13px;font-weight:600;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all .15s`)}>{o.label}</div>
            ))}
          </div>

          <div style={css(`margin-top:18px;font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Khối lớp`)}</div>
          ...
          <div style={css(`margin-top:18px;font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Tên lớp`)}</div>
          <div style={css(`margin-top:10px;padding:14px;border:1px solid #ddeaf0;border-radius:16px;background:#f8fcfd`)}>
            <div style={css(`font-size:11.5px;color:#455771`)}>{v.t(`Tên lớp sẽ được tạo tự động`)}</div>
            <div style={css(`font-size:17px;font-weight:700;color:#455771;margin-top:5px;letter-spacing:.02em`)}>{v.ccPreviewName}</div>
          </div>
      ...
      {v.isTCreateLecture && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          ...
          <div style={css(`margin-top:18px;font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Nội dung giáo án`)}</div>
          <div style={css(`margin-top:10px;padding:14px;border:1px solid #ddeaf0;border-radius:16px;background:#f8fcfd`)}>
            <div style={css(`font-size:11.5px;color:#455771`)}>{v.t(`Tiêu đề sẽ được tạo tự động`)}</div>
            <div style={css(`font-size:15px;font-weight:700;color:#455771;margin-top:5px`)}>{v.clPreviewTitle}</div>
          </div>
      ...
      {v.isTExplore && (
        <div style={css(`padding-bottom:120px;animation:ybup .3s ease`)}>
          ...
          <div style={css(`position:sticky;top:0;z-index:20;background:#fcfcfc;padding:14px 20px 12px;margin-top:6px;border-bottom:1px solid #ddeaf0`)}>
            <div style={css(`display:flex;align-items:center;gap:10px`)}>
              <div style={css(`flex:1;display:flex;align-items:center;gap:9px;height:46px;padding:0 14px;border:1px solid #ddeaf0;border-radius:14px;background:#fff`)}>
                ...
                <input value={v.search} onChange={(e) => v.setSearch(e.target.value)} placeholder={v.t(`Tìm kiếm học liệu`)} style={css(`flex:1;min-width:0;border:none;outline:none;background:transparent;font-size:14px;color:#455771`)}/>
                {!!v.search && (<svg onClick={() => v.setSearch("")} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8ba0ae" strokeWidth="2.2" strokeLinecap="round" style={css(`cursor:pointer;flex:none`)}><path d="M6 6l12 12M18 6L6 18"/></svg>)}
              </div>
          ...
          <div style={css(`padding:14px 20px 0;display:flex;align-items:center;justify-content:space-between`)}>
            <div style={css(`font-size:13px;color:#455771`)}><span style={css(`font-weight:600;color:#455771`)}>{v.resultCount}</span> {v.t(`học liệu`)}</div>
            <div style={css(`display:flex;align-items:center;gap:8px`)}>
              <div style={css(`position:relative`)}>
                <div onClick={v.toggleSortMenu} style={css(`display:flex;align-items:center;gap:5px;height:32px;padding:0 11px;border:1px solid #ddeaf0;border-radius:999px;background:#fff;font-size:12.5px;color:#25475a;cursor:pointer`)}>{v.exploreSortLabel}<svg width="9" height="6" viewBox="0 0 10 6" fill="none" stroke="currentColor" strokeWidth="1.6" style={{ transform: v.sortMenuOpen ? "rotate(180deg)" : "none" }}><path d="M1 1l4 4 4-4"/></svg></div>
                {v.sortMenuOpen && (
                  <>
                    <div onClick={v.closeSortMenu} style={css(`position:fixed;inset:0;z-index:39`)}></div>
                    <div style={css(`position:absolute;top:38px;right:0;z-index:40;background:#fff;border:1px solid #ddeaf0;border-radius:14px;box-shadow:0 10px 24px rgba(25,86,88,.14);overflow:hidden;min-width:136px`)}>
                      {v.sortMenuOptions.map((o) => (
                        <div key={o.key} onClick={o.onClick} style={css(`padding:10px 14px;font-size:12.5px;font-weight:${o.selected ? 600 : 400};color:${o.selected ? "#00aaab" : "#455771"};background:${o.selected ? "#eaf6f8" : "#fff"};cursor:pointer;white-space:nowrap`)}>{o.label}</div>
                      ))}
                    </div>
                  </>
                )}
          ...
          {!v.tExploreItems.length && (
            <div style={css(`margin:24px 20px 0;padding:28px 20px;border:1.4px dashed #ddeaf0;border-radius:20px;background:#f8fcfd;text-align:center`)}>
              <div style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Chưa có học liệu phù hợp`)}</div>
              <div style={css(`font-size:12px;color:#455771;margin-top:5px;line-height:1.5`)}>{v.t(`Thử chọn một chủ đề khác hoặc chọn lại "Tất cả".`)}</div>
            </div>
          )}
            ...
            {v.tExploreItems.map((l, i) => (
              <div key={i} style={css(`border:1px solid #ddeaf0;border-radius:18px;background:#fff;overflow:hidden;box-shadow:0 3px 12px rgba(25,86,88,.045)`)}>
                <div style={css(`height:88px;position:relative;background:${l.tintBg}`)}>
                  ...
                  <div style={css(`position:absolute;left:8px;top:8px;height:21px;padding:0 8px;border-radius:999px;background:rgba(255,255,255,.92);font-size:9.5px;font-weight:600;color:#455771;display:flex;align-items:center`)}>{l.typeLabel}</div>
                </div>
                <div style={css(`padding:10px 11px 11px`)}>
                  <div style={css(`font-size:12.5px;font-weight:600;color:#455771;line-height:1.35;text-wrap:pretty`)}>{l.title}</div>
                  <div style={css(`font-size:10.5px;color:#455771;margin-top:4px`)}>{l.meta}</div>
      ...
      {v.isTClass && (
        <div style={css(`padding-bottom:120px;animation:ybup .3s ease`)}>
          ...
          <div style={css(`padding:16px 20px 0`)}>
                ...
                <div style={css(`margin-top:11px;display:flex;flex-direction:column;gap:11px`)}>
                  {v.tNews.map((n, i) => (
                    <div key={i} style={css(`padding:14px;border:1px solid #ddeaf0;border-radius:18px;background:#fff`)}>
                      <div style={css(`display:flex;align-items:baseline;justify-content:space-between;gap:10px`)}><div style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{n.title}</div><span style={css(`font-size:10.5px;color:#455771;white-space:nowrap`)}>{n.when}</span></div>
                      <div style={css(`font-size:12.5px;line-height:1.6;color:#455771;margin-top:6px;text-wrap:pretty`)}>{n.body}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {v.tTabStudents && (
              <div style={css(`margin-top:14px`)}>
                <div style={css(`display:flex;align-items:center;justify-content:space-between;gap:10px`)}>
                  <span style={css(`font-size:12.5px;color:#455771`)}>{v.tClassStudents} {v.t(`học sinh`)}</span>
                ...
                <div style={css(`margin-top:12px;display:flex;flex-direction:column;gap:10px`)}>
                  {v.tStudents.map((st, i) => (
                    <div key={i} style={css(`display:flex;align-items:center;gap:12px;padding:13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff`)}>
                      ...
                      <div style={css(`flex:1;min-width:0`)}><div style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{st.name}</div><div style={css(`font-size:11.5px;color:#455771;margin-top:2px`)}>{st.meta}</div></div>
            ...
            {v.tTabWork && (
              <div style={css(`margin-top:14px`)}>
                <div style={css(`display:flex;justify-content:flex-end`)}><div onClick={v.toTCreateAssignmentForClass} style={css(`height:32px;padding:0 14px;border-radius:999px;background:#00aaab;color:#fff;font-size:12px;font-weight:600;display:flex;align-items:center;gap:5px;cursor:pointer`)}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"><path d="M12 5v14M5 12h14"/></svg>{v.t(`Tạo bài tập`)}</div></div>
                <div style={css(`margin-top:11px;display:flex;flex-direction:column;gap:11px`)}>
                  {v.tClassAssignments.map((a, i) => (
                    <div key={i} onClick={a.onClick} style={css(`padding:14px;border:1px solid #ddeaf0;border-radius:18px;background:#fff;cursor:pointer`)}>
                      <div style={css(`font-size:13.5px;font-weight:600;color:#455771;line-height:1.4;text-wrap:pretty`)}>{a.title}</div>
                      <div style={css(`font-size:11.5px;color:#455771;margin-top:5px`)}>{v.t(`hạn`)} {a.due} · {a.submitted} {v.t(`đã nộp`)}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div style={css(`margin-top:11px;display:flex;flex-direction:column;gap:10px`)}>
                  {v.tBlocks.map((b, i) => (
                    <div key={i} style={css(`display:flex;align-items:center;gap:12px;padding:13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff`)}>
                      ...
                      <div style={css(`flex:1;min-width:0`)}><div style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{b.title}</div><div style={css(`font-size:11.5px;color:#455771;margin-top:2px`)}>{b.kind} · {b.dur}</div></div>
                      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#8ba0ae" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9.5 5l7 7-7 7"/></svg>
                    </div>
                  ))}
                </div>
            ...
            {v.tTabSettings && (
              <div style={css(`margin-top:14px;display:flex;flex-direction:column;gap:18px`)}>
                <div>
                  <div style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Thông tin lớp học`)}</div>
                  <div style={css(`margin-top:10px;display:flex;flex-direction:column;gap:2px;border:1px solid #ddeaf0;border-radius:20px;background:#fff;overflow:hidden`)}>
                    {v.tClassInfo.map((r, i) => (
                      <div key={i} style={css(`display:flex;align-items:center;justify-content:space-between;padding:13px 16px;border-bottom:1px solid #edf7f9;gap:10px`)}>
                        <span style={css(`font-size:12.5px;color:#455771;flex:none`)}>{r.label}</span>
                        <span style={css(`font-size:13px;font-weight:600;color:#455771;text-align:right`)}>{r.value}</span>
                      </div>
                    ))}
                  </div>
                  <div style={css(`margin-top:10px;padding:13px 14px;border:1px solid #ddeaf0;border-radius:16px;background:#fff`)}>
                    <div style={css(`font-size:11.5px;color:#455771`)}>{v.t(`Mô tả lớp`)}</div>
                    <div style={css(`font-size:12.5px;line-height:1.6;color:#455771;margin-top:5px;text-wrap:pretty`)}>{v.tClassDesc}</div>
                  </div>
                </div>

                <div>
                  <div style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Quyền tham gia lớp`)}</div>
                  <div style={css(`margin-top:10px;display:flex;flex-direction:column;gap:9px`)}>
                    {v.tClassAccessOptions.map((o, i) => (
                      <div key={i} onClick={o.onClick} style={css(`display:flex;align-items:flex-start;gap:11px;padding:13px 14px;border:${o.selected ? "1.6px solid #00aaab" : "1px solid #ddeaf0"};background:${o.selected ? "#eaf6f8" : "#fff"};border-radius:16px;cursor:pointer;transition:all .15s`)}>
                        ...
                        <div style={css(`flex:1;min-width:0`)}>
                          <div style={css(`font-size:13px;font-weight:600;color:#455771`)}>{o.title}</div>
                          <div style={css(`font-size:11.5px;color:#455771;margin-top:3px;line-height:1.45;text-wrap:pretty`)}>{o.desc}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <div style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Giáo viên phụ trách`)}</div>
                  <div style={css(`margin-top:10px;display:flex;align-items:center;gap:11px;padding:13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff`)}>
                    ...
                    <div style={css(`flex:1;min-width:0;font-size:13.5px;font-weight:600;color:#455771`)}>{v.tClassMainTeacher.name}</div>
                  ...
                  <div style={css(`margin-top:14px;display:flex;align-items:center;justify-content:space-between`)}>
                    <span style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Giáo viên khác`)}</span>
                  ...
                  <div style={css(`margin-top:10px;display:flex;flex-direction:column;gap:9px`)}>
                    {v.tClassCoTeachers.map((c, i) => (
                      <div key={i} style={css(`display:flex;align-items:center;gap:11px;padding:12px 13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff`)}>
                        ...
                        <div style={css(`flex:1;min-width:0`)}>
                          <div style={css(`font-size:13px;font-weight:600;color:#455771`)}>{c.name}</div>
                          <div style={css(`font-size:11px;color:#455771;margin-top:2px`)}>{v.t(`Giáo viên`)} · {c.addedOn}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <div style={css(`font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Quyền truy cập của học sinh`)}</div>
                  <div style={css(`margin-top:10px;display:flex;flex-direction:column;gap:2px;border:1px solid #ddeaf0;border-radius:20px;background:#fff;overflow:hidden`)}>
                    {v.tClassPermRows.map((r, i) => (
                      <div key={i} style={css(`display:flex;align-items:center;gap:12px;padding:13px 16px;border-bottom:1px solid #edf7f9`)}>
                        <div style={css(`flex:1;font-size:12.5px;color:#455771;line-height:1.4`)}>{r.label}</div>
      ...
      {v.isTGrading && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          ...
          <div style={css(`margin-top:16px;display:flex;flex-direction:column;gap:10px`)}>
            <div style={css(`display:flex;align-items:center;justify-content:space-between;height:48px;padding:0 14px;border:1px solid #ddeaf0;border-radius:14px;background:#fff`)}><div><div style={css(`font-size:10.5px;color:#455771`)}>{v.t(`Lớp học`)}</div><div style={css(`font-size:13px;font-weight:600;color:#455771`)}>{v.t(`SINH HỌC 8A1`)}</div></div><svg width="11" height="7" viewBox="0 0 10 6" fill="none" stroke="#617789" strokeWidth="1.6"><path d="M1 1l4 4 4-4"/></svg></div>
            <div style={css(`display:flex;align-items:center;justify-content:space-between;height:48px;padding:0 14px;border:1px solid #ddeaf0;border-radius:14px;background:#fff`)}><div><div style={css(`font-size:10.5px;color:#455771`)}>{v.t(`Bài tập`)}</div><div style={css(`font-size:13px;font-weight:600;color:#455771`)}>{v.t(`Bài tập tuần 20: Quang hợp`)}</div></div><svg width="11" height="7" viewBox="0 0 10 6" fill="none" stroke="#617789" strokeWidth="1.6"><path d="M1 1l4 4 4-4"/></svg></div>
          </div>
          <div style={css(`margin-top:14px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px`)}>
            {v.tGradeSummary.map((g, i) => (
              <div key={i} style={css(`padding:13px 10px;border-radius:16px;background:${g.bg};text-align:center`)}><div style={css(`font-size:22px;font-weight:700;color:${g.color};line-height:1`)}>{g.value}</div><div style={css(`font-size:10.5px;color:#455771;margin-top:5px`)}>{g.label}</div></div>
            ))}
          </div>
          <div style={css(`margin-top:18px;font-size:14.5px;font-weight:600;color:#455771`)}>{v.t(`Bài nộp`)}</div>
          <div style={css(`margin-top:12px;display:flex;flex-direction:column;gap:10px`)}>
            {v.tSubmissions.map((x, i) => (
              <div key={i} onClick={x.onClick} style={css(`display:flex;align-items:center;gap:12px;padding:13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;cursor:pointer`)}>
      ...
      {v.isTGrade && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          ...
          <div style={css(`margin-top:9px;padding:14px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;font-size:13px;line-height:1.65;color:#455771;text-wrap:pretty`)}>{v.subAnswer}</div>
          <div style={css(`margin-top:14px;font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Tệp đính kèm`)}</div>
          <div style={css(`margin-top:9px;display:flex;align-items:center;gap:10px;padding:12px 13px;border:1px solid #ddeaf0;border-radius:14px;background:#fff`)}><div style={css(`width:30px;height:30px;border-radius:9px;background:#eaf6f8;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#00708f`)}>PDF</div><div style={css(`flex:1`)}><div style={css(`font-size:12.5px;font-weight:500;color:#455771`)}>{v.t(`so-do-quang-hop.pdf`)}</div><div style={css(`font-size:10.5px;color:#455771`)}>{v.t(`1,2 MB`)}</div></div></div>
          <div style={css(`margin-top:18px;font-size:13.5px;font-weight:600;color:#455771`)}>{v.t(`Điểm`)}</div>
          ...
          <div style={css(`margin-top:9px;padding:14px;border:1px solid #ddeaf0;border-radius:16px;background:#fff;min-height:86px;font-size:13px;line-height:1.6;color:#455771`)}>{v.t(`Bài làm trình bày rõ hai pha của quang hợp. Em bổ sung thêm kết luận về vai trò của quang hợp với sự sống nhé.`)}</div>
          {v.notGraded && (
      ...
      {v.isTPlans && (
        <div style={css(`padding:6px 20px 120px;animation:ybup .3s ease`)}>
          <div style={css(`font-size:22px;font-weight:700;color:#455771`)}>{v.t(`Giáo án của tôi`)}</div>
          <div style={css(`font-size:13px;color:#455771;margin-top:3px`)}>{v.t(`3 giáo án · 2 đã xuất bản`)}</div>
          ...
          <div style={css(`margin-top:16px;padding:16px;border-radius:20px;background:#fbf5ed;border:1px solid #eddfcc`)}>
            <div style={css(`font-size:14px;font-weight:700;color:#455771`)}>{v.t(`Kết nối YooStudio`)}</div>
            ...
            {v.tPlans.map((p, i) => (
              <div key={i} onClick={p.onClick} style={css(`padding:15px;border:1px solid #ddeaf0;border-radius:18px;background:#fff;cursor:pointer`)}>
                <div style={css(`display:flex;align-items:center;justify-content:space-between;gap:10px`)}>
                  ...
                  <span style={css(`font-size:11px;color:#455771`)}>{p.used}</span>
                </div>
                <div style={css(`font-size:14px;font-weight:600;color:#455771;line-height:1.4;margin-top:9px;text-wrap:pretty`)}>{p.title}</div>
                <div style={css(`font-size:11.5px;color:#455771;margin-top:5px`)}>{p.sub}</div>
              </div>
            ))}
            {!v.tPlans.length && (
              <div style={css(`margin-top:10px;text-align:center;font-size:13px;color:#455771`)}>{v.t(`Không có giáo án nào ở đây.`)}</div>
            )}
          </div>
        </div>
      )}
      ...
      {v.isTPlan && (
        <div style={css(`padding:6px 20px 40px;animation:ybup .3s ease`)}>
          ...
          <div style={css(`margin-top:20px;display:flex;align-items:baseline;justify-content:space-between`)}><div style={css(`font-size:14.5px;font-weight:600;color:#455771`)}>{v.t(`Nội dung giáo án`)}</div><span style={css(`font-size:12.5px;color:#00708f`)}>{v.t(`4 mục`)}</span></div>
          <div style={css(`margin-top:12px;display:flex;flex-direction:column;gap:10px`)}>
            {v.tBlocks.map((b, i) => (
              <div key={i} style={css(`display:flex;align-items:center;gap:12px;padding:13px;border:1px solid #ddeaf0;border-radius:16px;background:#fff`)}>
                <div style={css(`width:6px;height:38px;flex:none;border-radius:999px;background:${b.tint}`)}></div>
