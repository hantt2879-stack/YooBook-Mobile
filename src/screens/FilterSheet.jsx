...
function ChipRow({ items, scroll }) {
  return (
    <div className={scroll ? "yb-scroll" : ""} style={css(scroll
      ...
      {items.map((o, i) => (
        <div key={i} onClick={o.onClick} style={css(`flex:none;height:38px;padding:0 14px;border:${o.selected ? "1.6px solid #00aaab" : "1px solid #ddeaf0"};background:${o.selected ? "#eaf6f8" : "#fff"};color:${o.selected ? "#00708f" : "#25475a"};border-radius:999px;font-size:12.5px;font-weight:600;display:flex;align-items:center;justify-content:center;cursor:pointer;white-space:nowrap;transition:all .15s`)}>{o.label}</div>
      ))}
    </div>
  );
}
    ...
    <div style={css(`position:absolute;inset:0;z-index:80;display:flex;flex-direction:column;justify-content:flex-end`)}>
      ...
      <div style={css(`position:relative;max-height:82%;background:#fcfcfc;border-radius:24px 24px 0 0;box-shadow:0 -10px 30px rgba(0,0,0,.18);display:flex;flex-direction:column;animation:ybup .25s ease`)}>
        <div style={css(`padding:16px 20px 10px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #ddeaf0`)}>
          <div style={css(`font-size:16px;font-weight:700;color:#195658`)}>{v.t(`Bộ lọc`)}</div>
        ...
        <div style={css(`flex:1;overflow-y:auto;padding:16px 20px 8px;display:flex;flex-direction:column;gap:18px`)}>
          <div>
            <div style={css(`font-size:13px;font-weight:600;color:#195658;margin-bottom:9px`)}>{v.t(`Môn học`)}</div>
            <ChipRow items={v.filterSubjects} scroll />
          </div>
          <div>
            <div style={css(`font-size:13px;font-weight:600;color:#195658;margin-bottom:9px`)}>{v.t(`Cấp học`)}</div>
            <ChipRow items={v.filterLevels} />
          </div>
          <div>
            <div style={css(`font-size:13px;font-weight:600;color:#195658;margin-bottom:9px`)}>{v.t(`Khối lớp`)}</div>
            <ChipRow items={v.filterGrades} scroll />
          </div>
          <div>
            <div style={css(`font-size:13px;font-weight:600;color:#195658;margin-bottom:9px`)}>{v.t(`Loại học liệu`)}</div>
            <ChipRow items={v.filterTypes} />
          </div>
          <div>
            <div style={css(`font-size:13px;font-weight:600;color:#195658;margin-bottom:9px`)}>{v.t(`Bài / chủ đề`)}</div>
            <ChipRow items={v.filterTopics} scroll />
          </div>
          <div>
            <div style={css(`font-size:13px;font-weight:600;color:#195658;margin-bottom:9px`)}>{v.t(`Sắp xếp`)}</div>
